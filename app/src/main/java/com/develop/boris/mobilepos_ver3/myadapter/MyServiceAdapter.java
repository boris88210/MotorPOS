...
public class MyServiceAdapter extends MyBaseAdapter<Service> {

  private OnClickListener clickListener;
  private OnLongClickListener longClickListener;

  public MyServiceAdapter(Context context, ArrayList<Service> dataList, OnClickListener clickListener,
  ...
  @Override
  protected void rowSelected(Service song, int index) {
  ...
  public View getView(int position, View view, ViewGroup parent) {
    ...
    if (dataList.size() == 0) {
      ...
      // 取得這個位置的資料
      final Service data = dataList.get(position);
      final int index = position;

      viewHolder.title.setText(data.getName());
      viewHolder.root.setTag(R.id.adaptorLayout, data);
      viewHolder.root.setBackgroundColor(context.getResources().getColor(R.color.colorBackground));
